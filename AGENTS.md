...
bun run lint       # lint (oxlint --type-aware --deny-warnings src/)
...
7. Read module README and package.json before working in the respective directory.
8. If `product-spec.md` exists in the root - it is the source of truth for the feature set; read it before dispositioning new features.
9. Use `.docs/agents-audit.prompt.md` to verify that agent rules are not stale and still match the code.
...
If `.docs/` contains `{{...}}` placeholders - this is the first run. Read the installed `first-run.md` and execute it: fill the templates, then offer a deep analysis of the project or initialization of a new one. All strict rules apply during the first run as well.

## Available tools

At session start the agent must list available MCP servers and tools, and use them whenever the task matches: library docs via docs tools before answering from memory, browser tools for UI verification. Full rule: `.docs/AGENT_PROMPT.md`, section "Available tools". Do not overuse calls.