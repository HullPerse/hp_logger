...
export class ConsoleTransport implements Transport {
  ...
  private buffered: BufferedConsole | null = null;
  ...
  constructor(settings: ResolvedSettings) {
    ...
    if (settings.bufferedConsole) {
      this.buffered = new BufferedConsole((out, err) => {
        if (out !== null) consoleWrite("info", out, false);
        if (err !== null) consoleWrite("error", err, false);
      });
    }
  }
  ...
  private emit(level: LogLevel, output: string, debugTraceToConsoleDebug: boolean): void {
    ...
    this.buffered.push(level, output);
  }
